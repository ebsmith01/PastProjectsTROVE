...
```
When doing group by color, you can add in the custom ebs color scale like so, there is a palette for continuous groups and also one for discrete grouping. If you just want the standard ebs green in only one color, specify the fill(color for geom_point) in the geom.
```{r fig.align='center'}
head(iris)
...
ggplot(iris) + 
  geom_bar(aes(x = Sepal.Length), fill= ebsgreen) +
  ebs_black() # notice the fill color is set to ebs green within the geom, this makes the bars the standard ebs green

ggplot(iris) +
  geom_boxplot(aes(x = Sepal.Length,y= Sepal.Width), fill= ebsgreen, color = "#fafafa") +
  ebs_black() # for boxplots, you should specify the outline color and not worry about messing with the fill color

ggplot(iris) +
  geom_point(aes(x = Sepal.Length,y= Sepal.Width), color= ebsgreen) +
  ebs_black() #Note for point, you specify the color not the fill as ebs green
